...
colors:
  ...
  surface:         "#faf8f3"   # 側邊欄 / 次要區塊背景（米白暖灰）
  surface-soft:    "#fcfbf7"   # hover / 斑馬紋底
  primary-soft:    "#faf0ea"   # 選取態 / 資訊區塊淺硃紅底
  primary-tint:    "#f5ded3"   # 強調淺硃紅面（badge 底、active 邊）
  ink:             "#211d16"   # 主文字（暖調近黑）
  slate:           "#5c5546"   # 次要文字
  steel:           "#8f8776"   # 第三層文字 / placeholder（僅限大文字或非必要資訊）
  hairline:        "#e8e2d5"   # 邊框 / 分隔線
  hairline-strong: "#d3cab7"   # 輸入框邊框 / 較重分隔
  primary:         "#bf3b1b"   # 主色（硃砂紅）：按鈕 / 連結 / active 指示
  primary-pressed: "#992d10"   # 主色按下態
  on-primary:      "#ffffff"   # 主色上的文字
  success:         "#1a7f37"   # 已分析 ✓
  warning:         "#9a6700"   # abstract_only 警示 / 分析中
  warning-soft:    "#fbf4e3"   # 警示區塊淡底
  error:           "#d1242f"   # 失敗
...
文書風 × 暖白/硃砂紅「硃批」色盤（與行銷頁同源）。個人論文研究工具：長文閱讀優先、操作簡潔、無多餘裝飾元素——不搬行銷頁的紙紋/印章/動畫。

## 設計原則

1. **閱讀面純白**：keypoints / 比較表等長文內容一律 `canvas` 白底，最大化閱讀對比；米白 `surface` 只給側邊欄與輔助區塊。
2. **硃砂紅只做訊號**：`primary` 硃砂紅保留給可點擊（按鈕、連結）與目前位置（active 指示），不拿來裝飾。大面積永遠是白/米白。
...
### 警示區塊（abstract_only）
- background: `warning-soft`（warning 淡底）、border-left: `3px solid warning`
...
| 色對 | 對比度 | 要求 | 結果 |
|------|--------|------|------|
| ink on canvas | 16.4:1 | 4.5:1 | ✓ |
| ink on surface | 15.7:1 | 4.5:1 | ✓ |
| slate on canvas | 7.4:1 | 4.5:1 | ✓ |
| steel on canvas | 3.6:1 | 3:1（大文字） | ✓ |
| on-primary on primary | 5.4:1 | 4.5:1 | ✓ |
| primary on canvas（連結） | 5.4:1 | 4.5:1 | ✓ |
