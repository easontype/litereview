...
- **批判檢核（v1.7）**：審查同時回答批判性思考五問——研究問題是否明確定義、研究設計是否適合回答問題、樣本量是否足夠、資料收集是否可靠、作者是否承認研究限制——每問給通過／部分／未過判定與原文出處；另比對出「作者未承認的限制」（審查者發現、但論文隻字未提的弱點）。
- **辯論**：給定一個爭點（可從審查結果一鍵帶入），讓正反方模型針對論文證據多輪攻防，最後由裁判判決；逐字稿透過 SSE 即時呈現。
- **合議庭判決（v1.7）**：預設三位裁判獨立評分（可在設定中心把三席指到不同家模型，避免同源偏好），每位按四維 rubric（論點品質／證據運用／反駁力度／回應完整度）分別給正反雙方打分——總分與勝負由程式從分項計算，不是模型一句話說了算；多數決定勝方，一致度（3/3 或 2/1）就是這場結果的信心指標，另附程式統計的引文硬指標（每方引用次數／不重複引文數）。駁論階段強制「讓步協議」：先指出對方最強論點並表態，再反駁，避免雙方各說各話空轉。
- **外部證據庫（v1.9）**：發起辯論時可勾選啟用——系統從被辯論文的 **OpenAlex 引文圖譜**（它引用誰＋誰引用它）檢索文獻，只留 **Q1/Q2 期刊或 CORE A\*/A 會議**，再經 LLM 相關性過濾編成 6–8 張證據卡【X#】：摘要**逐字**收錄（不經改寫）、附期刊分級徽章／年份／DOI 與關聯註記。正反方與裁判拿到**同一份**證據庫，裁判可查驗雙方引用是否忠實；規則上外部證據只能支撐背景性主張，針對論文本身的數據攻防仍必須引用論文全文引文【E#】，濫用會在「證據運用」維度扣分。建庫約需 20–40 秒（SSE 回報進度），論文需有 DOI 或 arXiv ID；建庫失敗時自動降級為一般辯論，不影響進行。
- **對話（v1.8）**：側欄「對話」開聊天室，勾選至多 3 篇工作區論文把**全文注入脈絡**（每篇 60k 字截斷，UI 顯示實際注入的篇數與字數），與模型針對論文內容自由問答，回覆逐字串流；每個對話可就地切換 provider 與模型（清單來自設定中心，不影響八個座位的指派）。輸入 `/review`（1 篇）、`/compare`（≥2 篇）、`/debate <辯題>` 直接在對話中觸發既有流程，結果以摘要＋連結卡落地；支援附圖提問（限 API provider——claude-cli 走純文字 CLI，附圖按鈕自動隱藏）。
...
- **即時串流**：找重點／比較以背景 job 執行，SSE 回報階段進度（抓全文→LLM 分析→完成）與耗時，離開頁面再回來會自動接上進行中的 job；辯論逐字稿逐 token 即時顯示（v1.8 起 Anthropic／OpenAI 相容／Gemini API provider 也支援逐字串流，不再只有 claude-cli）。
...
路由分兩個 route group：`/`（marketing）是靜態商品介紹頁，`(app)` 群組（`/dashboard`、`/workspace`、`/chat`、`/compare`、`/debate`、`/journals`、`/graph`、`/pdfs`、`/settings`）才是側邊欄 App Shell。
...
5. 「辯論」頁選辯題與論文（1–3 篇）→ 可勾選「外部證據庫」（需論文有 DOI／arXiv ID）讓正反方引用引文圖譜檢索來的高分文獻 → 開始辯論，即時看正反方攻防與裁判判決；歷史在側邊欄「辯論」區塊
6. 「對話」頁（或 ⌘K「開新對話」）開聊天室：右上選模型、按「論文脈絡」勾選至多 3 篇注入全文，直接針對論文內容問答；輸入 `/` 會浮出命令選單（`/review`、`/compare`、`/debate <辯題>`），結果會以摘要＋連結卡回到對話裡
...
八個座位（找重點／比較／審查員／辯論正方／辯論反方／辯論裁判／裁判二／裁判三）可各自指定供應商與模型：
...
- 辯論建議讓反方用非同家模型（一方 Claude、一方 GPT/Gemini），打破同源模型互相回聲；三席裁判指到三家不同模型則能同時避免裁判的自我偏好——同模型跑三次也有價值（非確定性下的自我一致性），但異質合議的信心指標更有意義。
...
```bash
npm run build && OPENALEX_BASE_URL=http://localhost:3947 npx next start -p 3010   # 或用 npm run dev 開發模式
```

（`OPENALEX_BASE_URL` 是給 `v1.9-external-evidence.spec.ts` 用的——該 spec 會在本機 3947 起一個 OpenAlex fixture server，測試不打真 API；不跑 v1.9 spec 可省略。）
